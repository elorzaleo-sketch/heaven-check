...
- **Teléfono obligatorio** en la pantalla de datos personales (junto con nombre y empresa).
...
- Pantalla de resultado con título "Tu resultado" y un cierre más persuasivo ("No dejes pasar esta oportunidad…") con un solo botón — ya no vuelve a pedir el teléfono (usa el que la persona cargó al principio) y al confirmar muestra un popup de festejo ("Volver al inicio", pensado para que el/la vendedor/a reinicie el juego para la próxima persona) en vez de un formulario.
- **Ícono propio + "agregar a pantalla de inicio"**: tanto el juego como el panel ahora tienen el logo de Heaven como ícono de pestaña, y se pueden instalar como acceso directo en el celular/tablet (Android: menú del navegador → "Agregar a pantalla de inicio"; iPhone/iPad: botón Compartir → "Agregar a pantalla de inicio"). Se probó un service worker para que además apareciera el cartel de "Instalar app", pero causaba problemas guardando en Supabase en el mismo dispositivo — se sacó. El acceso directo con el ícono correcto sigue funcionando igual, solo que sin ese cartel automático.

### Si algún dispositivo se quedó con el problema de "no guarda"

Los dispositivos que ya habían visitado el juego con la versión que tenía el service worker pueden haberlo quedado instalado. El código nuevo lo desinstala solo en cuanto abren la página una vez más — pero si en algún dispositivo puntual sigue sin guardar después de eso, la forma de forzarlo es borrar los datos del sitio: en Chrome (Android), ⋮ → Información del sitio → Borrar datos del sitio; en Safari (iPhone/iPad), Ajustes → Safari → Avanzado → Datos de sitios web → buscar el dominio y eliminarlo. Después recargar la página una vez.
